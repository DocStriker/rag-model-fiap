...
`gemini_service.py` implementa um cache LRU de até 512 entradas, indexado pelo hash SHA-256 do texto concatenado com o nome do modelo. Isso elimina chamadas repetidas à API OpenAI para textos idênticos (queries frequentes, re-uploads do mesmo documento), reduzindo latência e custo.
...
`tests/test_api.py` cobre rotas HTTP (chat, collections, upload, health), lógica do `ChatService` (mensagem vazia, RAG sem resultados) e utilitários (`_chunk_text`). Os testes usam mocks completos para Gemini e Qdrant, rodando sem dependências externas via `pytest tests/ -v`.
